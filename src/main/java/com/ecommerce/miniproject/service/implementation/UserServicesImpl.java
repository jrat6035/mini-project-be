    ...
    public User getUserById(String id) {
        try {
            return userRepository.findById(id).orElseThrow(() ->
                    new Exceptions(ErrorMessages.ERROR_USER_NOT_FOUND, HttpStatus.NOT_FOUND));
        } catch (Exception exception) {
            LOGGER.error("Error while fetching user with id {}", id, exception);
            throw new Exceptions("Error while fetching users",
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public User updateUser(String id, User updatedUser) {
        try {
            User currentUser = userRepository.findById(id).orElseThrow(() ->
                    new Exceptions(ErrorMessages.ERROR_USER_NOT_FOUND, HttpStatus.NOT_FOUND));
            modelMapper.map(currentUser, updatedUser);
